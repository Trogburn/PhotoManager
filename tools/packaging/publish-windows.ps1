...
$project = Join-Path $repoRoot 'src\QnapPhotoManager\QnapPhotoManager.csproj'
$csproj = Get-Content -LiteralPath $project -Raw
if ($csproj -notmatch '<Version>([^<]+)</Version>') {
    throw "Could not read Version from $project"
}
$version = $Matches[1]
$zipName = "QnapPhotoManager-$version-$Runtime.zip"
$stage = Join-Path $OutputRoot "QnapPhotoManager-$version-$Runtime"
