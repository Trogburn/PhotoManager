[CmdletBinding()]
param(
    [string]$InstallRoot = (Join-Path $env:LOCALAPPDATA 'QnapPhotoManager'),
    [switch]$Launch
)
...
$exeSource = Join-Path $payloadRoot 'QnapPhotoManager.exe'
if (-not (Test-Path -LiteralPath $exeSource)) {
    throw "QnapPhotoManager.exe was not found next to this installer. Extract the zip first, then run Install.bat."
}
...
Write-Host "Installing Qnap Photo Manager to $InstallRoot"
Get-ChildItem -LiteralPath $payloadRoot -Force |
    Where-Object { $_.Name -notin @('Install.bat', 'Install-QnapPhotoManager.ps1') } |
    ForEach-Object {
        Copy-Item -LiteralPath $_.FullName -Destination (Join-Path $InstallRoot $_.Name) -Recurse -Force
    }

$installedExe = Join-Path $InstallRoot 'QnapPhotoManager.exe'
...
$shortcutPath = Join-Path $programs 'Qnap Photo Manager.lnk'
$shell = New-Object -ComObject WScript.Shell
$shortcut = $shell.CreateShortcut($shortcutPath)
$shortcut.TargetPath = $installedExe
$shortcut.WorkingDirectory = $InstallRoot
$shortcut.Description = 'Review-first QNAP photo duplicate and date workflow'
$shortcut.Save()
