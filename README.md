...
Windows users who only want the photo app can download `QnapPhotoManager-*-win-x64.zip` from a GitHub Release, extract it, and run `Install.bat`. That install does not require this repository. macOS is not supported. See [src/QnapPhotoManager/OPERATIONS.md](src/QnapPhotoManager/OPERATIONS.md).
