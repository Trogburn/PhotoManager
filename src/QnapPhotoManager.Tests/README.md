# QnapPhotoManager tests

This xUnit project validates the WPF application's non-UI safety boundaries.
It uses only temporary directories and does not connect to a QNAP share,
invoke Czkawka, or modify repository media.

Run it from the repository root:

```powershell
dotnet test .\src\QnapPhotoManager.Tests\QnapPhotoManager.Tests.csproj -c Release
```
...
The separate `QnapPhotoManager.UiTests` project contains FlaUI desktop
...
```powershell
dotnet test .\src\QnapPhotoManager.UiTests\QnapPhotoManager.UiTests.csproj -c Release
```
